...
左ツリーから **新規 Markdown ファイルを作成** できるようになりました (Issue #6)。ターミナルに戻って `touch` する必要はもうありません — ツールバーの「＋」やディレクトリの「＋」から作成して、そのまま編集モードで書き始められます。
...
- `tests/server.test.ts`: 作成系の正常 / 異常系 12 件 (`.md`/`.markdown`/`.mdx` 作成、サブディレクトリ、409、トラバーサル、絶対パス、拡張子、親不存在、除外ディレクトリ、JSON 不正、Origin 403、405)
- `tests/new-file.test.ts`: ファイル名補完ロジック (`public/new-file.js`、pure module) のユニットテスト 11 件
