...
- **スマホ幅の全画面 TOC が `Esc` で閉じるようになった**: 全画面パネル（`z-index: 60`）なのに閉じる導線が `×` ボタンしか無く、他の全画面パネルと作法が揃っていなかった。あわせて `OVERLAY_LAYERS` に登録し、重なったときの優先順位を他と同じ仕組みで決めるようにした（クイックオープンより手前には来ない ——実際の描画順に合わせた）

## [0.21.1] - 2026-08-12

**`yomi down` が無関係なプロセスを終了させることがあった問題を修正しました** (Issue #132)。pid だけで本人と判断していたのをやめ、**プロセスの起動時刻を突き合わせて同定する**ようにしています。副産物として、**応答しなくなった yomi も `down` で止められる**ようになりました。

**この版へ上げたら、動いている yomi を一度起動し直してください。** 記録に起動時刻が入るのは次の起動からで、それまでは「そのプロセスが yomi か」を見る弱い判定に落ちます。

### Fixed (Issue #132)

- **`yomi down` が、無関係なプロセスを終了させることがあったのを修正した**: yomi が異常終了して記録が残り、その pid を OS が別のプロセスに再利用し、さらに別の何かがそのポートを掴んでいると、**記録が無関係なプロセスを指す**。そこへ `yomi down` が SIGTERM を送ったうえで「停止しました」と報告していた。**プロセスの起動時刻を突き合わせて同定する**ようにしたので、pid が再利用されていても本人でなければシグナルを送らない。`yomi list` と起動前の検査も同じ判定を使う
- **応答しなくなった yomi を `yomi down` で止められるようになった**: これまでは「そのポートで応答しているか」を同定の条件にしていたため、ハングして listen が落ちた yomi は**記録だけ消えて本体が生き残って**いた。`down` が最も要るのはこの状況なので、同定から疎通を外した
- **記録に起動時刻を残すようになった**（`procStartedAt`）。**v0.21.0 以前の記録にはこの項目が無い**が、その場合も「そのプロセスがそもそも yomi か」を見るので、無関係なプロセスは止めない。yomi を起動し直せば起動時刻が入り、同定が効くようになる
- **`yomi list` が、応答しない yomi の記録を消さなくなった**: 一覧には出さないが記録は残すので、そのあと `yomi down` で止められる。これまでは**`list` を打っただけで生きた yomi が孤児になって**いた

## [0.21.0] - 2026-08-12

**記事の画像をまとめて持ち出せるようになり、大きなツリーの表示が速くなりました。** 表示中の Markdown が参照する画像を 1 回の操作で zip にできます (Issue #140)。ツリーの再描画は差分更新になり、10,000 ファイルで **216ms → 18ms**（`/api/tree` は 19.6ms → 0.6ms）になりました (Issue #84)。`.yomiignore` に否定パターン (`!node_modules`) を書いて既定除外を解除できます (Issue #97)。

**外へ出る情報の穴を 4 つ塞いでいます。** 除外配下の実在が応答の差から漏れる問題 (Issue #98)、500 応答に絶対パスやスタックが載る問題 (Issue #99)、ポートを奪われたときのスタックトレース (Issue #107)、そして `..cache` のような**正当な名前が読めなかった**問題 (Issue #118)。保存は temp + rename で原子化し (Issue #101)、その差し替えを watcher が取りこぼしていた件も直しました (Issue #119)。
