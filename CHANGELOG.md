...
- **現行実装のベースラインを記録した**: `docs/bench/tree-baseline.md`。**DOM 更新が支配的**で、10,000 ファイルでスキャン 20ms に対し描画 178ms（約 9 倍）。差分更新（#84）の効果が最も大きいのはクライアント側と見込まれる
- **計測はプロダクトコードを変えていない**: 既存の `scanMarkdownTree` / `createServer` を呼ぶだけで、注入点の追加もしていない
