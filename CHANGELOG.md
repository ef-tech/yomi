...
- **同じファイルを続けて保存したとき、余計なリロードが飛ぶことがあったのを修正した**: 保存の失敗時にパス単位でマークを無条件に消していたため、**同じファイルを保存中の別リクエストのマーク**まで巻き添えにしていた。巻き添えになったリクエストの保存は watcher から「他人の変更」に見え、編集中の画面が再読み込みされる。**自分が立てたマークだけを消す**ようにした（`SaveMark.clear` が sha を必須にしたので、値を見ない削除はもう書けない）
