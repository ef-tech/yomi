...
- **上書き保存でファイル監視のイベントが飛ばなくなった**: chokidar は「既存ファイルへの rename」で `change` を発火しない（実測）。**自分の保存が二重にリロードされることは元々無かった**ので利用者から見た挙動は変わらないが、**同じディレクトリを開いている別の yomi インスタンスは保存を検知しなくなる**。これは外部エディタが atomic save したときと同じ状態で、検知そのものは Issue #119 で扱う
