...
- **上書き保存をファイル監視が取りこぼすことがある**: chokidar は inotify で一時ファイルを認識してから rename を関連付けるため、**その処理が終わる前に rename すると変更イベントが出ない**。実測（Linux / Bun 1.3.12 / chokidar 5、各 20 回）では、一時ファイルの書き込みから rename までの間隔が 5ms 以上なら 20/20 で検知され、0ms だと 3/20 まで落ちる。この保存経路は間を空けないため 15/20 だった。**自分の保存は元々二重リロードされない**ので利用者から見た挙動は変わらず、実害は**同じディレクトリを開いている別の yomi インスタンスが 1/4 ほど取りこぼす**ことに限られる。**外部エディタの atomic save は間隔が閾値を超えるため影響を受けない**。取りこぼしそのものは Issue #119 で扱う
