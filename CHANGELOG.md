...
- **ファイル監視をポーリングに倒さなかった**: 監視側の設定（`atomic` / `alwaysStat` / `awaitWriteFinish`）では改善せず、確実なのは `usePolling` だけだった。ただし**アイドル時の CPU が 5,000 ファイルで 7 倍**（10 秒あたり 62ms → 455ms）になり、ファイル数に比例して増える。**保存 1 回あたり 5ms** のほうが安いと判断した（計測値は `.ef/verify/issue-119/REPORT.md`）
