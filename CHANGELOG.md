...
**左ツリーからファイルとディレクトリを削除できるようになりました** (Issue #171)。行にマウスを乗せる（または `Tab` でフォーカスする）と「×」が出ます。ディレクトリは**配下ごと**消えるので、**何が消えるか**（Markdown / その他のファイル / ディレクトリの件数）を数えて確認ダイアログに出してから削除します。

⚠️ **`--share` で LAN 公開している場合は、閲覧者がファイル・ディレクトリを削除できます。** Markdown の保存・新規作成・`.yomiignore` の編集 (#164) と同じ扱いで、README が宣言している「LAN 内の全員が信頼できる」前提に含まれます。信頼できない相手がいるネットワークでは `--share` を使わないでください。

### Added (Issue #171)

- **ツリーからファイルを 1 個削除できる**: `POST /api/file/delete`（ツリーに載るファイルが対象）
- **ツリーからディレクトリを配下ごと削除できる**: `POST /api/dir/delete`。非 Markdown・画像も一緒に消える
- **削除前に内訳を数えて確認する**: `GET /api/dir/delete` は**何も消さずに**配下の件数だけを返す。確認ダイアログに対象パス・配下の件数・非 Markdown を含む旨が出て、キャンセルすれば何も消えない
- **内訳は除外を適用せずに数える**: 除外 (`.yomiignore`) で隠れているファイルも削除では消えるため、数から落とすと確認ダイアログが実態より少なく見える
- **削除ボタンは hover と `Tab` フォーカスで出す**: クリックはツリー 1 つのリスナへ委譲しているので、行数が増えてもリスナは増えない
- **削除後にツリーが更新される**: 開いていたファイルを消したら画面に伝え、編集中なら編集モードを抜ける
- エラーコード `not_a_file` / `not_a_dir` / `delete_failed` を追加し、ja / en 双方の文言を用意した

### Security (Issue #171)

- **symlink は辿らない**: 親までを解決して leaf は綴りのまま繋ぎ、**リンク自体**を `unlink` する。リンク先の実体は消えない（ディレクトリへのリンクも中身は消えない）
- **ルート自身は削除できない**: `{"path":"."}` は `resolveSafe` を通ってしまうため明示的に弾く
- **除外配下 (`.yomiignore` / `DEFAULT_EXCLUDES`) は拒否する**: 解決前の字句と解決後の `rel` の 2 段で見る
- 削除リクエストは `checkOrigin` を通る

### Fixed (開発基盤)

- **macOS CI で間欠的に落ちていたテストを直した**（落ちるテストが毎回入れ替わる形。タイムアウトを伸ばしただけの対症療法ではなく、待ちの前提そのものの誤りを直した）
  - `tests/daemon.test.ts`: `startForeground` が **listen だけ**を待っており、`bin/yomi.ts` が記録を書き終える前に `readInstances()` が 0 件を返していた（二重起動対策で記録を `createServer` より後に書くため、listen から記録まで実測で数百 ms 空く）。**記録が現れるまで待つ**ようにし、失敗メッセージでも「listen したが記録が無い」を区別して出す
  - `tests/watcher.test.ts`: 実 chokidar のテストが 1 つの root を使い回しており、後半ほど初期スキャンと FSEvents の配送が重くなっていた。**テストごとに root を切り**、macOS の配信遅延を見込んで待ちを 3s → 10s に伸ばした

## [0.23.0] - 2026-09-03

**除外設定 (`.yomiignore`) を画面から編集できるようになりました** (Issue #164)。左ツリー上部の ⚙ から開くパネルで編集して保存すると、**yomi を再起動せずに**除外が差し替わります。「書いたのに効いていない行」も、これまで起動時の stderr にしか出ていませんでしたが、同じ文言でパネルに出るようになりました。

⚠️ **`--share` で LAN 公開している場合は、閲覧者がこの設定を書き換えられます。** 除外は v0.19.0 (Issue #65) 以降「読み書きの可否を決めるゲート」なので、`!node_modules` や `!.env` のような否定を書かれると**起動ディレクトリ配下の読み取り範囲が広がります**。Markdown の保存・新規作成と同じ扱いで、README が宣言している「LAN 内の全員が信頼できる」前提に含まれます。信頼できない相手がいるネットワークでは `--share` を使わないでください。

### Added (Issue #164)

- **`.yomiignore` を画面のパネルから編集・保存できる**: 左ツリー上部の ⚙ から開く。`Ctrl/Cmd+Enter` でも保存、`Esc` で閉じる。ファイルが無ければ保存時に作られる
- **保存すると再起動なしで除外が差し替わる**: ツリー・`/api/file`・`/api/asset`・`/api/images.zip`・ファイル監視の**全経路が同時に**新しい集合を見る。chokidar は張り終えた watch を作り直さないので、除外を解除したディレクトリを監視へ戻すために watcher ごと張り直している
- **保存すると開いている全ブラウザのツリーが更新される**: 除外集合は丸ごと入れ替わるため、差分ではなく全量取り直しを通知する
- **照合できない行をパネルに表示する**: 行番号つきで、**起動時の警告と同じ判定・同じ文言**（判定も文言も `src/yomiignore.ts` が正本で、画面側に写しを置かない）。捨てた行と、除外としては生きている行（グロブ文字を含む名前）を見た目で分ける
- **表示中のファイルが除外配下になったら知らせる**: 保存直後にツリーから消えた場合、パネル内に警告を出す

### Changed (Issue #164)

- **`.yomiignore` が読めなかったときに除外を空へ倒さなくなった**: 以前はあらゆる読み取り失敗を「除外なし」に倒していた。起動時は「0 件追加」の表示で気づけたが、**画面から差し替えるようになると、読めなかったときに利用者の除外が黙って消える**（＝読み取り範囲が広がる）。ファイルが無いときだけ空にし、それ以外は起動時なら警告して既定のまま続行、保存時なら**除外を差し替えずに** 500 (`reload_failed`) を返す
- **ルート外を指す symlink の `.yomiignore` を読まなくなった**: 起動時・画面のどちらからも拒否する（以前は起動時だけ読んでいたため、サーバは適用しているのに画面からは確認も修正もできない状態になりえた）
**プレビュー内のコードブロックをワンクリックでコピーできるようになりました** (Issue #165)。コードブロックにマウスを乗せると右上に ⧉ が出ます。押すとそのブロックの中身だけが生テキストで入るので、範囲選択で隣の見出しや区切り線を巻き込むことがありません。

### Added (Issue #165)

- **コードブロックにコピーボタンを出す**: 中身は `code.textContent` から取るので、シンタックスハイライトの `<span>` は混ざらない。押した手応えはパスのコピー (#24) と同じ見せ方に揃えた
- **Mermaid には出さない**: 図として描画されるため（`<pre class="mermaid">` は `<code>` を持たないので、`pre > code` で選ぶだけで自然に外れる）
- **読み取り専用のテキストファイル表示 (#155) にも出す**: DOM の形が同じ `pre > code` なので同じ扱いにした。押すとファイル全体が入る
- **スマホ・タブレットでは常時表示**: ホバーが無い環境では隠さない（`@media (hover: hover)` で PC だけホバー表示に切り替える）
- **HTTP（非セキュアコンテキスト）でも動く**: `--share` で LAN の別端末から開いたときも、既存のフォールバック（非表示 textarea + `execCommand`）を通る
- ボタンは**サニタイズ後の DOM に後付け**する。HTML 文字列へ混ぜるとサニタイザ (#21 / #59) の許可設定を広げることになるため
- **コピーされるのは画面に出ている文字だけ**: `textContent` は `display: none` の子孫まで拾うので、`<span hidden>` を仕込んだ md が「見えていないコマンド」を混ぜて貼らせられた。描画されている部分だけを集める
- ⚠️ **プレビューの DOM 構造が変わりました**: コードブロックの `<pre>` が `<div class="code-block">` で包まれます（ボタンを横スクロールで流さないため）。プレビューに自前の CSS を当てている場合は `.markdown-body pre` のような子孫セレクタなら影響ありませんが、直下セレクタ（`> pre`）を使っていると当たらなくなります

### Changed (開発基盤)

- **同梱している mermaid を 11.16.1 → 11.17.2、DOMPurify を 3.4.13 → 3.4.14 に更新した**（いずれも `public/vendor/` にバンドル済みなので、利用者側の操作は不要）
- **ef-devkit の足場を追加した**: Issue / PR テンプレート、ラベル定義、GitHub Projects の自動連携ワークフロー、CI の証跡検査を配置した。利用者向けの挙動は変わらない
- **PR 本文の証跡検査を CI 本体から切り離した** (`.github/workflows/pr-evidence.yml`): 証跡は PR 本文だけを見るので本文を編集したら再判定が要るが、`ci.yml` に `edited` を足すと (1) workflow レベルの `concurrency` が **走行中の test / E2E を打ち切り**、(2) `if` で飛ばした job も **check-run を publish して pass の記録を上書きする**（どちらも実測）。workflow を分けると `edited` で `ci.yml` が起動しないので、どちらも起きない
- 開発用依存を更新した: `@types/jsdom` 28.0.3 → 30.0.0

## [0.22.1] - 2026-08-16

**symlink 越しに許可リスト外のファイルが読めたのを塞ぎました** (Issue #156)。ドキュメントディレクトリの中に `note.md → .env` のような symlink が置かれていると、`/api/file` から `.env` の中身がそのまま返り、`/api/asset` からはバイナリが配信されていました。v0.22.0 で `.env` を許可リストから外した判断が、symlink 1 本で迂回されていた形です。

**影響はディレクトリ内のファイルに限られます**（ルート外は従来どおり拒否）。既定の `127.0.0.1` バインドなら他人からは触れませんが、**`--share` で LAN に公開している場合は更新をおすすめします**。

### Fixed (Issue #156)

- **symlink 越しに許可リスト外のファイルが読めたのを塞いだ**: `/api/file` と `/api/asset` は「要求されたパス」の拡張子だけを検査しており、**実際に読むファイル**は見ていなかった。ルート内に `note.md → secret.bin` のような symlink があると、許可リスト外の実体が Markdown としてレンダリングされて返る（`/api/file`）か、`application/octet-stream` として配信される（`/api/asset`）。**解決後のパスにも同じ許可リストを掛ける**ようにした（除外判定を解決前後の両方で掛けている Issue #65 と同じ形）
- 影響は**ルート内のファイルに限られる**（`resolveSafe` がルート外を拒否する）。既定では `127.0.0.1` バインドなので他人からは触れないが、`--share` で LAN 公開しているときはルート内の symlink 経由で読まれうる
- `/api/images.zip` も同じ形に揃えた（こちらは中身を返さないので情報は漏れないが、3 経路で関門の形を揃えないと、次に触る人がどれが正しいか判断できない）
- **`/api/asset` に残っていた誤ったコメントを直した**: 「要求側で検査済みなので解決後の拡張子は必ず許可リストにある（この fallback は到達しない）」と書かれていたが、symlink では成り立っていなかった

## [0.22.0] - 2026-08-16

**md 以外のファイルも yomi で読めるようになりました** (Issue #155)。`README.md` の隣にある `package.json` や `compose.yaml`、設定ファイル、コードを見るためだけにエディタへ切り替える必要がありません。左ツリーにそれらも並び、クリックすると**読み取り専用**で開いて、拡張子に応じたシンタックスハイライトが付きます。

編集・保存・新規作成は**これまでどおり Markdown だけ**です。テキストを開いているあいだは編集ボタンが押せなくなり、ヘッダに「読み取り専用」と出ます。

### Added (Issue #155)

- **md 以外のテキストファイルも左ツリーに載せ、読み取り専用で開けるようにした**: md の隣に置かれた `.json` / `.csv` / 設定ファイル / コードを見るためだけにエディタへ切り替える必要がなくなった。表示は許可リスト方式で、`.txt` / `.csv` / `.json` / `.yaml` / `.toml` / `.xml` / `.html` / 主要なコード拡張子と、`Dockerfile` / `Makefile` / `.gitignore` のような拡張子を持たない慣習ファイルを対象にする
- **シンタックスハイライトを付けた**: highlight.js を配布物へ同梱する（Issue #52 と同じく CDN を参照しない）。全部入りではなく、許可リストに出てくる 29 言語だけを登録して 151 KB に収めた。配色は CSS 変数でライト / ダーク両方に定義する
- **読み取り専用に留めた**: 編集・保存・新規作成は従来どおり Markdown だけ。テキストを開いているあいだは編集ボタン・目次・画像 zip が押せなくなり、ヘッダに「読み取り専用」と出る（パスのコピーは使える）。表示モードは「プレビュー」に固定する —— 並列 / MD は「ソースとプレビュー」を出し分けるもので、raw しか持たないテキストでは同じ中身が 2 つ並ぶだけになるため
- **ライブリロードとクイックオープンの対象にも入れた**: ツリーに載る集合と監視する集合を 1 か所（`src/util/viewable.ts`）で決めるようにしたので、「ツリーに出るのにライブリロードされない」ずれが生まれない
- **中身を HTML として解釈しない**: 本文は `textContent` で入れ、ハイライト結果も DOMPurify を通してから差し込む（Issue #21 / #59 の関門を迂回しない）。`.html` を開いてもタグは文字として表示される
- **テキストには 2 MB のサイズ上限を設けた**（超過は 413）。中身を丸ごとブラウザへ渡して色付けするため。Markdown はこれまでどおり上限なし
- `.env` は秘密情報を含むことが多いため既定の許可リストに入れていない（`.yomiignore` の否定パターンで各自が解除できる）

## [0.21.3] - 2026-08-14

**`yomi` を叩くと README が開くようになりました** (Issue #150)。これまでは「ツリー最初のファイル」を開いており、ツリーはディレクトリを先に並べるので、**必ずいちばん深いディレクトリの中のファイル**が出ていました。「なぜこのファイルが開くのか」が分からないうえ、その祖先ディレクトリが全部開いた状態で始まります。ルートに README があれば、それを入口にします。

### Changed (Issue #150)

- **起動時にルート直下の `README.md` を開くようにした**: これまでは「ツリー最初のファイル」を開いていたが、ツリーはディレクトリを先に並べるため、それは**必ずいちばん深いディレクトリの中のファイル**になっていた（`yomi .` を叩いた利用者から見て「なぜこのファイルが開くのか」が分からず、しかもその祖先ディレクトリが全部開いた状態で始まる）。ルートに README を置く慣習は広いので、あればそれを入口にする
- 綴りは**大文字小文字と拡張子を問わない**（`readme.md` / `README.MD` / `README.markdown` / `README.mdx`）。ツリーに載るのは yomi が Markdown とみなしたファイルだけなので、ここだけ `.md` に絞る理由が無い
- **探すのはルート直下だけ**で、`docs/README.md` のような下位の README は優先しない。`?path=` を指定した場合はこれまでどおりそちらが優先される（共有したリンクが README に飛ばされない）
- ルート直下に README が無ければ、**従来どおりツリー最初のファイル**を開く

## [0.21.2] - 2026-08-13

**大きなツリーの表示が目に見えて軽くなりました。** ファイルの追加・削除をサーバが差分で通知するようになり、10,000 ファイルで **1 イベントあたり 18.2ms → 0.0ms** になっています (Issue #126)。ついでに、**新しく作ったファイルが一覧に出ないことがあった問題**も直しました（v0.20.0 以降の穴）。

**スマホで全画面の目次が `Esc` で閉じるようになりました** (Issue #135)。`×` ボタンしか無く、他の全画面パネルと作法が揃っていませんでした。

**同じファイルへの並行保存で、余計なリロードが飛ぶことがあったのを直しました** (Issue #129)。編集中だと事実でない競合バナーが出ることもありました。

**開発者向け**: `bun test` がフル実行時に間欠的に落ちていた問題を直し (Issue #133)、フェイクサーバが実サーバと違う並びのツリーを返していたのも直しました (Issue #145)。
...
- **取りこぼしても壊れないようにした**: `/api/tree` は `X-Yomi-Tree-Gen` ヘッダで版を返す。差分の版が「手元 + 1」でなければ**全量を取り直す**ので、通知が 1 通落ちても・途中から接続しても表示がずれない。版を返さないサーバや `--depth` 指定時は、従来どおり全量を取り直す

### Fixed (Issue #126)

- **新しく作られたファイルが一覧に出ないことがあったのを修正した**: `writeFile` 1 回でも chokidar は `add` の直後に `change` を出すことがあり、80ms の debounce が**最後の種類で上書きして「追加」を「内容変更」に化けさせて**いた。v0.20.0 以降 `changed` はツリーを取り直さないので、**そのファイルは次に別の追加・削除が起きるまで一覧に出ない**（`bun test` のフル実行で実際に踏んだ）。構造の変化と内容の変化を別々に覚えるようにして塞いだ

### Fixed (Issue #129)

- **同じファイルへの並行保存で、余計なリロードが飛ぶことがあったのを修正した**: 自己保存マークが 1 パス 1 値だったため、後から来た保存が**先の保存のマークを消して**いた。先の保存が watcher から「他人の変更」に見え、保存した本人の画面にリロードが飛ぶ（編集中なら**事実でない競合バナー**が出る）。**1 パスに複数のマークを持てる**ようにして塞いだ。#120 は「消す側」だけを直したもので、この経路は残っていた
- マークは **1 パスあたり 4 件**まで保持する（保存が成功したマークは誰も消さないので、上限が無いと溜まり続ける）。副作用として、**外部エディタが「直近 4 回のどれか」と同じ内容へ戻したとき**の誤抑止の窓が、従来の「直前の 1 回」から広がる
...
- **フェイクサーバが実サーバと違う並びのツリーを返していたのを直した**（テスト基盤の問題で、プロダクションコードは変えていない）: `defaultTree()` がファイルをディレクトリより前に置いており、`src/scanner.ts` の `sortTree`（ディレクトリが先、その中で名前順）と食い違っていた。差分更新は「渡された子リストが整列済み」を前提にするので、**実装が正しいのに位置がずれて見える**（#126 の実装中に実際に踏んだ）。逆に**実物で壊れていても気づけない**方向にも効く
- **fixture が実サーバとずれていないことを機械的に守るようにした**: `defaultTree()` が挙げたパスで実ファイルを作り、`scanMarkdownTree` に走らせた結果と突き合わせる（並び順だけでなく `pruneEmpty` の意味論まで守れる）
