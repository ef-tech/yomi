 ...
 * メモリリーク防止のため LRU で上限を持つ（既定で**パス 64 × 1 パス 4 件 = 最大 256 件**。
 * sha は 64 文字なので 20KB 弱に収まる）。
 *
...
export class SaveMark {
   ...
   * 2. リクエスト 2 がマーク B を立てる
   * 3. リクエスト 1 が失敗して**パスごと**消す → **B まで消える**
   ...
   *
   * **#129 より前は 2 の時点で A が失われていた**（1 パス 1 値だったので B が A を
   * 上書きしていた）。いまは両方が残るので、ここで消すのは**自分の 1 件だけ**で済む。
   *
