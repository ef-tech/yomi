...
- **ツールバーの「＋」**: ルート直下に作成
- **ディレクトリ行に hover で出る「＋」**: そのディレクトリの子として作成
- インライン入力欄が開くので、ファイル名を入力して `Enter` で確定、`Esc` でキャンセル
- 拡張子は省略可 (`foo` → `foo.md`)。`.markdown` / `.mdx` を明示した場合はそのまま
- 作成に成功すると新規ファイルが選択され、そのまま編集モードで開く
