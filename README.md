...
現状はディレクトリ/ファイル名の完全一致のみ。グロブ (`*`, `**`) は未対応です。書式には次の 3 つの限界があります（除外は読み書きの拒否にも使われるため、いずれも意図せず「防げていない」状態になり得ます）:

- **`/` を含むパス指定は効きません。** `private/creds.csv` のように書いても、照合はパスの各セグメントとの完全一致で行うためどのセグメントにも一致せず、**警告も出ないまま無効**になります。`private`（ディレクトリ名）か `creds.csv`（ファイル名）のように 1 セグメントで書いてください
...
既定の除外パターン（`node_modules` / `dist` / `build` / `vendor` など）を**解除する手段は現状ありません**。`.yomiignore` は既定集合に追加するだけで、否定パターン (`!name`) は未対応です（→ #97）。
