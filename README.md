...
**除外はツリーの表示だけでなく読み取りにも効きます (Issue #65)。** 除外したパスは `/api/file`（Markdown 本文）からも `/api/asset`（画像・添付ファイル）からも取得できず、URL を直接叩いても 400 で拒否されます。除外配下のファイルは実在しても存在しなくても同じ応答になるため、存在の有無も分かりません。除外ディレクトリに置いた画像を Markdown から参照している場合は表示されなくなります（v0.20.0 での変更点。それ以前は取得できていました）。
