...
- ブラウザ内 Markdown 編集（Ctrl/Cmd+S で保存）と外部エディタ起動の両対応
...
- **保存**: `Ctrl/Cmd+S`、または「編集」ボタン横の「完了」を押して確認
- **未保存表示**: ヘッダに `● 未保存` が点灯。タブを閉じようとすると警告
- **外部エディタで開く**: 「外部エディタ」ボタン押下で `$EDITOR` / `code` を起動
...
引数つきも書ける (`YOMI_EDITOR="code --wait"` 等)。
