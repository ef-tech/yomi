...
- 左ツリーからのファイル / ディレクトリ削除（消える内訳を出す確認ダイアログ付き）
...
### コードブロックをコピーする (Issue #165)

プレビュー内のコードブロックにマウスを乗せると、右上に **⧉** ボタンが出ます。押すとそのブロックの中身が**生テキストのまま**クリップボードへ入ります（シンタックスハイライトのタグは混ざりません）。

- **スマホ・タブレットでは常に表示**します（ホバーが無いため）。PC ではホバーとキーボードフォーカスで出ます
- **Mermaid のブロックには出ません**（図として描画されるため）
- **読み取り専用で開いたテキストファイル**（`.json` / `.ts` など。Issue #155）にも出ます。押すとファイル全体が入ります
- `--share` で LAN の別端末から HTTP で開いたときも動きます（`navigator.clipboard` が使えない環境向けのフォールバックを通ります）
...
**画面からも編集できます（Issue #164）。** 左ツリー上部の ⚙ ボタンで設定パネルが開き、`.yomiignore` をそのまま編集して「保存して反映」を押すと、**yomi を再起動せずに**除外が差し替わります。ツリー・本文の読み取り・添付の配信・ファイル監視のすべてが同時に新しい除外を見ます。

- ファイルが無ければ保存時に作られます
- **照合できない行はパネル上に行番号つきで出ます**（起動時の stderr と同じ判定・同じ文言）。「書いたのに効いていない」に画面だけを見ていて気づけるようにするためです
- `Ctrl/Cmd+Enter` でも保存できます（`Esc` で閉じる）
- 保存すると開いている全ブラウザのツリーが更新されます
...
左ツリーからファイルとディレクトリを削除できます。**取り消せません**（OS のゴミ箱には入りません）。

- **行の「×」**: マウスは hover、キーボードは `Tab` フォーカスで表示（「＋」と同じ出し方）
- **ファイル**: パスを示した確認ダイアログが出て、OK で削除
- **ディレクトリ**: **配下ごと**消えます。確認ダイアログには**消える内訳**（Markdown / その他のファイル / ディレクトリの件数）が出ます。**除外設定 (`.yomiignore`) で画面に出ていないファイルも一緒に消える**ので、件数はツリーの見た目ではなく**実際に消えるもの**を数えた値です（20,000 件まで数え、超えた場合はその旨を表示）
- **シンボリックリンクはリンクだけが消えます**（リンク先は残る）。ディレクトリへのリンクでも中身は消えません
- 削除したものを開いていた場合はヘッダにその旨が出て、編集モード中なら編集モードを抜けます
- 除外ディレクトリ (`node_modules` 等、`.yomiignore` 含む) 配下・起動ディレクトリの外・起動ディレクトリ自身は、サーバ側で拒否されます
- ツリーに出ない拡張子（画像など）はファイル単体では削除できません。ディレクトリごと消したときは一緒に消えます
...
- ⚠️ **`--share` 中は、閲覧者が画面から除外設定 (`.yomiignore`) を書き換えられます（Issue #164）。** 除外は読み書きの可否を決めるゲートなので、`!node_modules` や `!.env` のような否定を書かれると**起動ディレクトリ配下の読み取り範囲が広がります**。Markdown の保存・新規作成と同じ扱いで、上記「LAN 内の全員が信頼できる」前提に含まれます。信頼できない相手がいるネットワークでは `--share` を使わないでください
