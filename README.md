...
長時間稼働した yomi が、**接続はできるが応答が返らず、Ctrl+C でも `kill` でも止まらない**状態に陥ることがあります（実測で 7 日以上の稼働後に発生）。メインスレッドが内部ロックで停止して event loop に戻らなくなるのが原因で、シグナルハンドラも event loop から呼ばれるため Ctrl+C も届きません。
...
**このメッセージが出たら [#91](https://github.com/ef-tech/yomi/issues/91) に貼ってもらえると助かります。** 再現条件が絞れておらず、スレッドの状態が唯一の手がかりです。バックグラウンド起動（`yomi up -d`）の場合は状態ディレクトリのログに出ます。
