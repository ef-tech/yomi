...
なお `--depth` はこれとは別で、**読み取りを制限しません**。`tree -L` と同じ「起動時に走査する深さの上限」で、深さを超えたディレクトリもツリーには残ります（中を見ていないだけ）。浅い Markdown から深い Markdown への内部リンクは従来どおり辿れます。見せたくないものは `--depth` ではなく `.yomiignore` を使ってください。
